...
const projects = [
  {
    ...
    detail: [
      ...
      "The billboard went live on a Tuesday morning. By Friday, the CEO had interview requests from Bloomberg and CNBC. Brand awareness metrics among their target demographic jumped from 12% to 41% within the campaign's two-week run.",
    ],
  },
  {
  ...
  },
  {
    ...
    detail: [
      "A rapidly scaling consultancy with 12 global offices was suffering from brand fragmentation \u2014 every regional team was interpreting the visual identity differently. They needed a single source of truth that teams from Singapore to Sao Paulo could apply consistently.",
  ...
  },
  {
    ...
    detail: [
      ...
      "Within a week of publication, the founder received three additional press inquiries from TechCrunch, Business Insider, and a podcast with a 100K+ listener base. The Forbes mention became a permanent credibility anchor, referenced in the company's press kit, investor updates, and LinkedIn headline.",
    ],
  },
  {
...
export const Route = createFileRoute("/portfolio")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Real case studies from real brands. See the work that made our clients impossible to ignore." },
    ],
  }),
        ...
        <div className="flex flex-col items-start gap-6 md:flex-row md:items-end md:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-6 max-w-lg text-lg text-cream/70">
              Nine projects. Real brands. Measurable noise. From Times Square billboards to Forbes features, this is what happens when strategy meets volume.
            </p>
          </div>
          <Link
            to="/contact-us"
            className="inline-flex items-center gap-2 rounded-full bg-gold px-6 py-3.5 text-sm font-bold uppercase tracking-wider text-navy-deep shadow-gold transition hover:scale-[1.03]"
          >
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((p) => (
            <Link
              ...
              to="/portfolio/$slug"
              params={{ slug: p.slug }}
              className="group relative overflow-hidden rounded-2xl bg-cream/5 ring-1 ring-cream/10 transition hover:-translate-y-1 hover:ring-gold"
            >
              <div className="relative h-56 overflow-hidden">
                ...
                <div className="absolute right-4 top-4 rounded-full bg-gold/90 p-2 text-navy-deep opacity-0 transition group-hover:opacity-100">
                  <Eye className="h-4 w-4" />
                </div>
              </div>
              <div className="p-6">
                <p className="text-xs font-bold uppercase tracking-[0.25em] text-gold">{p.category}</p>
                <h3 className="mt-2 font-display text-xl font-bold text-cream group-hover:text-gold transition-colors">{p.title}</h3>
                <p className="mt-3 text-sm leading-relaxed text-cream/65">{p.description}</p>
                <div className="mt-5 inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-gold">
